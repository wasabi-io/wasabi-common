...
export default class Objects {
     ...
     * @returns {number}
     */
    public static getLength<T extends ObjectsProps>(src: T): number {
        let i = 0;
        if(!has(src)) return i;
        for(let key in  src) {
     ...
     * @return {T}
     */
    public static remove<T extends ObjectsProps>(src: T, key: any): T {
     ...
     * @return {T}
     */
    public static removeValue<T extends ObjectsProps>(src: T, value: any): T {
        if(!has(src)) return src;
        for(let key in  src) {
     ...
     * @returns {Array<T>}
     */
    public static map<T extends ObjectsProps, M extends ObjectsProps>(obj: Object, callback: MapCallback ): any[] {
        let result = [];
     ...
     * @returns {boolean}
     */
    public static forEach<T extends ObjectsProps>(obj: T, callback: ForEachCallback): boolean {
     ...
     * @returns {string[]}
     */
    public static getKeys<T extends ObjectsProps>(src: T): string[] {
        let keys: string[] = [];
     ...
     * @return {T}
     */
    public static addValue<T extends ObjectsProps>(src: T, key: string, value: any, keys?: string[]): T {
        if(keys && keys.length > 0) {
            let prop = src[key];
            if(!Assertions.isObject(prop)) prop = src[key] = {};
            let childKey = keys[0];
            keys.shift();
     ...
     * @returns {T}
     */
    public static getValue<T extends ObjectsProps>(src: T, key: string, keys?: string[]): T  {
        let prop = src[key];
     ...
     * @return {T}
     */
    public static clone<T extends ObjectsProps>(src: T, ignoreList?: string[]): T {
     ...
     * @returns {any}
     */
    public static merge<S extends ObjectsProps, D extends ObjectsProps>(src: S, dest: D, ignoreList?: string[]): S | D {
     ...
     * @return {any}
     */
    public static mergeDefaults<D extends ObjectsProps, P extends ObjectsProps>(defaults: D, props: P): D | P {
